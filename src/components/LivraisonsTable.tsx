  ...
  const filteredAndSortedLivraisons = useMemo(() => {
    let filtered = livraisons.filter(livraison => {
      const matchesSearch = !searchTerm || 
        livraison.parcelle?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        livraison.local_id?.toString().includes(searchTerm) ||
          ...
          <thead className="bg-gray-50">
            <tr>
              <th
                ...
                onClick={() => handleSort('date_pesee')}
              >
                Date {sortField === 'date_pesee' && (sortDirection === 'desc' ? '↓' : '↑')}
              </th>
